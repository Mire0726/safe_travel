...
var UserWhere = struct {
	...
	LocalID:   whereHelperstring{field: "`users`.`local_id`"},
...
func (r *userR) GetCreatedByEvents() EventSlice {
	if r == nil {
		return nil
	}
	return r.CreatedByEvents
}
...
var (
	userAllColumns            = []string{"id", "local_id", "name", "email", "created_at", "updated_at"}
	userColumnsWithoutDefault = []string{"id", "local_id", "name", "email", "created_at", "updated_at"}
...
// CreatedByEvents retrieves all the event's Events with an executor via created_by column.
func (o *User) CreatedByEvents(mods ...qm.QueryMod) eventQuery {
	...
	queryMods = append(queryMods,
		qm.Where("`events`.`created_by`=?", o.ID),
	)
...
func (userL) LoadCreatedByEvents(ctx context.Context, e boil.ContextExecutor, singular bool, maybeUser interface{}, mods queries.Applicator) error {
	...
	query := NewQuery(
		qm.From(`events`),
		qm.WhereIn(`events.created_by in ?`, argsSlice...),
	)
	if mods != nil {
		mods.Apply(query)
	}
